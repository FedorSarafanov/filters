...
\begin{document}
	\begin{tikzpicture}
	\draw[xshift=0mm,-latex] (-4,0) -- (4,0)
	node [anchor=west]
	{$\theta$};
	\draw[xshift=0mm,-latex] (0,0) -- (0,6)
	node [anchor=south]
	{$\omega$};
	\draw [thick,xshift=0mm, y=1cm, x=1cm,
	declare function={w(\t)=2*sin(deg(\t/2))+sqrt(4*sin(deg(\t/2))^2+4);}] plot [domain=-3.14:3.14, samples=3000,
	smooth] (\x,{w(\x)});
	\draw [thick,xshift=0mm, y=1cm, x=1cm,
	declare function={w(\t)=-2*sin(deg(\t/2))+sqrt(4*sin(deg(\t/2))^2+4);}] plot [domain=-3.14:3.14, samples=3000,
	smooth] (\x,{w(\x)});
	\draw [dashed] (-3.14,0) node[below] {$-\pi$}--(-3.14,5)
	 (3.14,0)node[below] {$\pi$}--(3.14,5)
	 (-4,0.82)--(4,0.82) (-4,4.85)--(4,4.85);
	\draw (0,0.82) node[below right] {$\omega_1$}
		   (0,4.85) node[above right] {$\omega_2$}
		   (0,1.9)node[right] {$\omega_0$};
	\end{tikzpicture}
\end{document}