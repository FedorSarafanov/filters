...
\begin{document}
	...
	declare function={w(\t)=3*sqrt(1-(\t*0.4)^2);}] plot [domain=0:2.50, samples=3000,
	smooth] (\x,{w(\x)});
	\draw [dashed](0,3) node [left]{$\rho$}--(3,3);
	\draw (2.5,0) node[below]{1};
	\draw [dashed,thick,xshift=0mm, y=1cm, x=1cm,
	declare function={w(\t)=3*sqrt((\t*0.4)^2-1);}] plot [domain=2.5001:4, samples=1000,
	smooth] (\x,{w(\x)});
	\end{tikzpicture}
\end{document}