...
\begin{document}
	...
	declare function={w(\t)=\t/sqrt((\t*0.4)^2-1);}] plot [domain=3:5, samples=3000,
	smooth] (\x,{w(\x)});
	\draw [dashed](0,2.5) node [left]{$\rho$}--(5,2.5)
	 (2.5,0) node [below]{1}--(2.5,4);
	\draw [dashed,thick,xshift=0mm, y=1cm, x=1cm,
	declare function={w(\t)=\t/sqrt(1-(\t*0.4)^2);}] plot [domain=0:2.2, samples=1000,
	smooth] (\x,{w(\x)});
	\end{tikzpicture}
\end{document}